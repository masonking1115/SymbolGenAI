...
def build_block_rules():
    ...
    R += [
        _sim("BLK_BIAS_FS_CEILING", block=B, sim_block="opa_bias", sim_type="dc_sweep",
             ...
             source="Bias",
             quote="Programmable range 0–640 µA, step size ~1 µA",
        ...
        _sim("BLK_BIAS_COMPLIANCE_0V5", block=B, sim_block="opa_bias", sim_type="dc_compliance",
             ...
             source="Bias", quote="nominally 320 µA at 0.5 V",
        ...
        _sem("BLK_BIAS_ACCURACY_BUDGET", block=B, sev="WARNING", sheet="bias", refdes="R40",
             ...
             source="Bias", quote="Programmable range 0–640 µA, step size ~1 µA … nominally 320 µA at 0.5 V",
        ...
        _sim("BLK_BIAS_LOOP_STABILITY", block=B, sim_block="opa_bias", sim_type="ac_stability",
             ...
             source="Bias",
             quote="op-amp output drives a PMOS gate; PMOS source ties to the op-amp − terminal",
        ...
        _sim("BLK_BIAS_POR_FAILSAFE", block=B, sim_block="opa_bias", sim_type="por_failsafe",
             ...
             source="Bias", quote="Off by default.",
             datasheet=("design_implementation.md", "Assembly / provisioning notes / MCP4728 EEPROM",
                        "the DAC code at the rail (0xFFF with VREF=VDD → V_OUT=3.3 V → PMOS off)"),
        ...
        _sem("BLK_BIAS_DAC_VREF_EXTERNAL", block=B, sev="WARNING", sheet="bias", refdes="U40",
             ...
             source_doc="design_implementation.md",
             source="Bias circuit — as built",
             quote="external VREF = 3.3 V for rail-to-rail",
    ...
    R += [
        _sim("BLK_LDO_SETPOINT_COVERAGE", block=B, sim_block="ldo_rail", sim_type="setpoint_coverage",
             ...
             source="LDO",
             quote="0.6–1.0 V output, set via ANY-OUT inputs connected to the FPGA.",
        ...
        _sim("BLK_LDO_LINE_REG", block=B, sim_block="ldo_rail", sim_type="line_regulation",
             ...
             source="Power", quote="From the FMC: 3.3 V (3P3V)",
        ...
        _sim("BLK_LDO_DC_OK", block=B, sim_block="ldo_rail", sim_type="dc_op_point",
             ...
             source="System block diagram (from the deck)",
             quote="The LDO generates Bobcat's low rails",
        ...
        _sem("BLK_LDO_EN_PULLDOWN", block=B, sev="ERROR", sheet="power", net="LDO_EN",
             ...
             source="LDO", quote="EN input from the FPGA with a 10 kΩ pull-down",
        ...
        _sem("BLK_LDO_PG_PULLUP", block=B, sev="WARNING", sheet="power", refdes="U10",
             ...
             source_doc="design_implementation.md",
             source="FMC LPC pinout / Power & management pins", quote="D1 | PG_C2M | LDO PG output",
        ...
        _sem("BLK_LDO_SHARED_RAIL_INTENT", block=B, sev="INFO", sheet="power", refdes="U10",
             ...
             source_doc="design_implementation.md",
             source="Design decisions / history",
             quote="one TPS7A8401A feeds VDDD/VDDA1/VDDA2 through 3×1×2 jumpers, all tapping the same LDO output bus",
    ...
    R += [
        _sem("BLK_LOADSW_DEFAULT_OFF", block=B, sev="ERROR", sheet="power", net="LSW_EN",
             ...
             source="Load switch", quote="Enable input from the FPGA with a 10 kΩ pull-down",
        ...
        _sem("BLK_LOADSW_VIN_RANGE", block=B, sev="WARNING", sheet="power",
             ...
             source="Load switch", quote="VADJ 1.2–3.3 V",
        ...
        _sem("BLK_LOADSW_VDDIO_PATH", block=B, sev="WARNING", sheet="power", rail="+VDDIO",
             ...
             source="FMC interface", quote="VADJ → Bobcat VDDIO through a load switch and a 1×2 jumper",
    ...
    for blk, net, st, sev, series_r, crit, title in pdn:
        R.append(_sim(f"BLK_PDN_{blk.split('_')[0].upper()}_DROOP",
                      ...
                      title=title, source="Bobcat (DUT) — pinout & connections",
                      quote="Decoupling capacitors on VDDD, VDDIO, VDDA1, VDDA2.",
    ...
    for blk, r_ref, rail in (("vdda1_pdn", "R20", "VDDA1"), ("vdda2_pdn", "R21", "VDDA2")):
        R.append(_sem(f"BLK_PDN_{rail}_SERIES0R", block=blk, sev="WARNING", sheet="bobcat", refdes=r_ref,
                      ...
                      source="Bobcat (DUT) — pinout & connections", quote="Series 0 Ω on VDDA1, VDDA2.",
    ...
    R += [
        _sem("BLK_EEPROM_SUPPLY", block=B, sev="WARNING", sheet="eeprom",
             ...
             source="EEPROM", quote="3.3 V supply, I²C, 8 Kbit",
        ...
        _sem("BLK_EEPROM_I2C_PULLUPS", block=B, sev="WARNING", sheet="eeprom", net="SCL",
             ...
             source="FMC interface", quote="I²C (SCL/SDA) → EEPROM and Bias",
             datasheet=("design_implementation.md", "Assembly / provisioning notes / I²C pull-ups",
                        "R60/R61 (2.2 kΩ to +3V3) are local SCL/SDA pull-ups"),
    ...
    R += [
        ...
        _sem("BLK_LDO_SETPOINT_NAMING", block="ldo_rail", sev="ERROR", sheet="power", refdes="U10",
             ...
             source="LDO",
             quote="TPS7A8401A (named in the deck).",
        ...
        _sem("BLK_FOOTPRINT_MATCHES_DATASHEET", block="opa_bias", sev="WARNING", sheet="bias",
             ...
             source_doc="design_implementation.md",
             source="Design-review findings / F-6",
             quote="footprint vs datasheet … MSOP-10 / VSSOP-8 / SOT883",
        ...
        # GROUNDING: VITA 57.1 LPC standard — every unlabeled C/D/G/H pin is GND
        # (ASP-134606-01 is the connector); our as-wired build ties all 61. Those
        # GND pins MUST return to the board GND, not float (core EE: a connector's
        # ground return path).
        _sem("BLK_FMC_GND_PINS_TIED", block="ldo_rail", sev="ERROR", sheet="fmc", refdes="J1",
             ...
             source_doc="design_implementation.md",
             source="FMC LPC pinout — as wired",
             quote="All other C/D/G/H pins are GND per the standard — 61 GND pins (all wired to the GND net",

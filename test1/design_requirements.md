...
- One TPS7A8401A feeds VDDD, VDDA1, and VDDA2 via 1×2 jumpers — confirm a single rail at one ANY-OUT setpoint is acceptable for all three (vs. one LDO per rail for independent setpoints during bring-up).
- Confirm decoupling cap values per rail.
- FMC pin assignments within the LA bank are TBD — to be selected to minimize trace crossings during pinning.
